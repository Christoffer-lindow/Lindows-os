...
#define LINDOWS_FAT16_SIGNATURE 0x29
#define LINDOWS_FAT16_FAT_ENTRY_SIZE 0x02
#define LINdOWS_FAT16_BAD_SECTOR 0xFF7
#define LINDOWS_FAT16_UNUSED 0x00
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t diretory_start_sector)
{
    struct fat_directory_item item;
    ...
    int res = 0;
    int i = 0;
    int directory_start_pos = diretory_start_sector * disk->sector_size;
    struct disk_stream *stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, directory_start_pos) != LINDOWS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while (1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != LINDOWS_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk *disk, struct fat_private *fat_private, struct fat_directory *directory)
{
    ...
    struct disk_stream *stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != LINDOWS_ALL_OK)
    {
        res = EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != LINDOWS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk *disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != LINDOWS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != LINDOWS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * LINDOWS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        if (entry == LINdOWS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
void fat16_fat_free_item(struct fat_item *item)
{
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name)
{
    ...
    char tmp_filename[LINDOWS_MAX_PATH];
...
struct fat_item *fat16_get_directory_entry(struct disk *disk, struct path_part *path)
{
    ...
    while (next_part != 0)
    {
        ...
        fat16_fat_free_item(current_item);
